...
public struct HexSelfOperatingManual: Sendable {
  ...
  public var text: String {
    ...
    Coding workflow, when the session tools are present: use process_start for builds, tests, servers and direct REPLs; process_read observes their durable output and actual exit. A successful start is not a completed command. Use workspace_apply_patch with observed revisions for edits, then workspace_changes to review staged, unstaged, pre-existing and untracked changes. A repeated identical process_start within one task requires a confirmed earlier exit and a later completed patch that actually changed a file. Legacy process_run retains its stricter replay fence. Do not add arbitrary argument changes to bypass it. Input acceptance does not prove command success. A pending/unknown input, partial patch, interrupted process or missing cleanup needs inspection, never blind replay. Task sessions stop on completion/cancellation; retained servers remain until stopped or their deadline. Pause and steering keep sessions alive. Process logs and patch receipts are scoped to the conversation and survive closing the UI; resident restart records interrupted sessions without restarting commands. The UI's Processes and changes control opens their output, input controls and Git review. Git review does not establish authorship for command/user edits and never stages, commits or pushes.
