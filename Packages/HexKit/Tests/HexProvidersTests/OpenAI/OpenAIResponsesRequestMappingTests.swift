...
struct OpenAIResponsesRequestMappingTests {
  ...
  @Test(arguments: [OpenAIResponsesService.platformAPI, .chatGPTCodexSubscription])
  func emptyToolTextPreservesTheStructuredActionReceipt(service: OpenAIResponsesService) throws {
    let call = ToolCall(
      id: ToolCallID(rawValue: "call_empty_receipt"), name: "lookup_weather", arguments: [:])
    let output: JSONValue = .object([
      "content": .array([.object(["type": .string("text"), "text": .string("")])]),
      "hex_browser_verification_required": .boolean(true),
    ])
    let result = ToolResult(
      toolCallID: call.id, status: .success, output: output, content: [.text("")])
    let configuration = try OpenAIResponsesConfiguration(
      service: service, models: [OpenAIResponsesTestFixture.model()])
    ...
    let plan = try OpenAIResponsesRequestBuilder(configuration: configuration).build(
      request, serverState: nil, localState: nil)
    let body = try JSONDecoder().decode(JSONValue.self, from: plan.body)
    guard case .object(let fields) = body, case .array(let input)? = fields["input"],
      case .object(let functionOutput)? = input.last,
      case .array(let content)? = functionOutput["output"], content.count == 1,
      case .object(let text) = content[0], case .string(let encoded)? = text["text"]
    else {
      Issue.record("Expected only the structured receipt, without empty rich text")
      return
    }
    #expect(functionOutput["type"] == .string("function_call_output"))
    #expect(functionOutput["call_id"] == .string(call.id.rawValue))
    let receipt = try JSONDecoder().decode(JSONValue.self, from: Data(encoded.utf8))
    #expect(receipt == .object(["status": .string("success"), "output": output]))
  }
