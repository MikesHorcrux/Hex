...
struct SQLiteJournalMigrationTests {
  ...
  func versionZeroDatabaseBuildsCurrentSchema() async throws {
    ...
    #expect(try JournalTestSupport.userVersion(at: configuration.databaseURL) == 7)
    #expect(
  ...
  func futureSchemaFailsWithoutMutatingDatabaseContents() async throws {
    ...
    do {
      ...
      #expect(error == .futureSchemaVersion(found: 8, supported: 7))
    }

    #expect(try JournalTestSupport.userVersion(at: databaseURL) == 8)
