...
struct SQLiteJournalMigrationTests {
  ...
  func versionZeroDatabaseBuildsCurrentSchema() async throws {
    ...
    #expect(try JournalTestSupport.userVersion(at: configuration.databaseURL) == 3)
    #expect(
  ...
  func futureSchemaFailsWithoutMutatingDatabaseContents() async throws {
    ...
    do {
      ...
      #expect(error == .futureSchemaVersion(found: 4, supported: 3))
    }

    #expect(try JournalTestSupport.userVersion(at: databaseURL) == 4)
