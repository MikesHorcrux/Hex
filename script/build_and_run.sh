...
readonly CANONICAL_BUILD_DIR="$(/usr/bin/plutil -extract 0.buildSettings.TARGET_BUILD_DIR raw -o - - <<< "$HEX_BUILD_SETTINGS")"
readonly EXPECTED_TEAM_ID="$(/usr/bin/plutil -extract 0.buildSettings.DEVELOPMENT_TEAM raw -o - - <<< "$HEX_BUILD_SETTINGS")"
if [[ ! "$EXPECTED_TEAM_ID" =~ ^[A-Z0-9]{10}$ ]]; then
    echo "Set DEVELOPMENT_TEAM in Config/Developer.local.xcconfig before building Hex." >&2
    exit 1
fi
readonly RESIDENT_KEYCHAIN_GROUP="${EXPECTED_TEAM_ID}.com.lunarmothstudios.Hex.resident"
readonly APP_CODE_SIGNING_REQUIREMENT="anchor apple generic and identifier \"${BUNDLE_ID}\" and certificate leaf[subject.OU] = \"${EXPECTED_TEAM_ID}\""
