...
Use an isolated feature branch and keep module changes focused. Changes to the Xcode project,
package manifest, scripts, entitlements, or shared contracts require careful review because they
cross module boundaries. Coordinate overlapping work in the pull request. Maintainers review and
merge changes into the public default branch after verification.

## Runtime and release boundaries

Building is separate from enabling the background agent. Do not activate services, start provider
logins, download models, or change macOS privacy permissions as a hidden build side effect.
Development signing must preserve the app/helper shared identity checks. Distribution packaging,
notarization, and repository publication are explicit release operations.

Developer signing configuration is local to the checkout. The app and helper derive the runtime
team boundary from their validated signatures; no contributor must inherit the maintainer's team.
