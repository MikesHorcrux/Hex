...
extension AgentSQLiteConversationStore {
  func importLegacyArchive() async throws {
    ...
    guard let legacy else { throw ConversationStorageFailure.unavailable }
    let archive =
    ...
    var receipts: [ConversationStorageReceipt] = []
    for original in archive.conversations {
      ...
      for kind in [ConversationStorageEntryKind.display, .message, .exchange, .compaction] {
        var before: Int64?
        repeat {
