  ...
  var body: some View {
    HStack(alignment: .center, spacing: 12) {
      if work.phase == .running || work.phase == .pausing || work.phase == .cancelling {
        ProgressView().controlSize(.small).tint(HexBrandPalette.coral)
      }
      VStack(alignment: .leading, spacing: 4) {
        Text(work.phase == .running ? "Hex is working" : work.explanation)
          .font(.callout).foregroundStyle(HexBrandPalette.mutedInk).accessibilityIdentifier(
      ...
      if work.phase == .paused {
        Button("Resume", systemImage: "play") { Task { await model.control(.resume) } }
      } else if work.phase != .blocked && work.phase != .cancelling {
        Button("Pause", systemImage: "pause") { Task { await model.control(.pause) } }.disabled(
          work.phase == .pausing)
      }
      Button("Cancel", role: .destructive) { Task { await model.control(.cancel) } }
        .disabled(work.phase == .cancelling)
    }.buttonStyle(.borderless).disabled(model.isSubmitting || model.pending != nil)
  }
}
